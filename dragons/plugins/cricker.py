...
from . import drgub, edit_or_reply, reply_id
...
async def _(event):
    ...
    drgevent = await edit_or_reply(event, "```Gathering info...```")
    async with event.client.conversation(chat) as conv:
        try:
            ...
            await drgevent.edit("Unblock @cricbuzz_bot & try again")
            return
        if respond.text.startswith("I can't find that"):
            await drgevent.edit("sorry i can't find it")
        else:
