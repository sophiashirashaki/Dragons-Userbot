...
@drgub.drg_cmd(
    ...
    info={
        ...
        "examples": "{tr}exec drg stringsetup.py",
    },
)
async def _(event):
    ...
    drgevent = await edit_or_reply(event, "`Executing.....`")
    process = await asyncio.create_subprocess_shell(
        cmd, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE
    )
    stdout, stderr = await process.communicate()
    result = str(stdout.decode().strip()) + str(stderr.decode().strip())
    drguser = await event.client.get_me()
    curruser = drguser.username or "drguserbot"
    uid = os.geteuid()
    if uid == 0:
...
@drgub.drg_cmd(
    ...
    info={
        ...
        "examples": "{tr}eval print('drguserbot')",
    },
)
async def _(event):
    ...
    drgevent = await edit_or_reply(event, "`Running ...`")
