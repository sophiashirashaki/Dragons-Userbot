...
from . import drgub, edit_delete, edit_or_reply
...
async def get_adzan(adzan):
    ...
    if request.status_code != 200:
        return await edit_delete(adzan, f"`Couldn't fetch any data about the city {LOKASI}`", 5)
    result = json.loads(request.text)
    drgresult = f"<b>Islamic prayer times </b>\
    ...
    "
    await edit_or_reply(adzan, drgresult, "html")
