# Heroku manager for your Dragons-Userbot
...
async def variable(var):  # sourcery no-metrics
    ...
    if exe == "get":
        drg = await edit_or_reply(var, "`Getting information...`")
        await asyncio.sleep(1.0)
        try:
            variable = var.pattern_match.group(2).split()[0]
            if variable in heroku_var:
                return await drg.edit(
        ...
        drg = await edit_or_reply(var, "`Setting information...`")
        if not variable:
            return await drg.edit("`.set var <ConfigVars-name> <value>`")
        ...
        if variable in heroku_var:
            await drg.edit(f"`{variable}` **successfully changed to  ->  **`{value}`")
        else:
            await drg.edit(
        ...
        drg = await edit_or_reply(var, "`Getting information to deleting variable...`")
        try:
            variable = var.pattern_match.group(2).split()[0]
        except IndexError:
            return await drg.edit("`Please specify ConfigVars you want to delete`")
        await asyncio.sleep(1.5)
        if variable not in heroku_var:
            return await drg.edit(f"`{variable}`**  does not exist**")

        await drg.edit(f"`{variable}`  **successfully deleted**")
...
async def dyno_usage(dyno):
    ...
    dyno = await edit_or_reply(dyno, "`Checking your dyno usage...⚡`")
    useragent = (
