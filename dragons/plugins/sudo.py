...
@drgub.drg_cmd(
    ...
    info={
        "header": "To enable or disable sudo of your Dragons-Userbot.",
...
async def chat_blacklist(event):
    "To enable or disable sudo of your Dragons-Userbot."
...
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "__There are no sudo users for your Dragons-Userbot.__"
        )
    result = "**The list of sudo users for your Dragons-Userbot are :**\n\n"
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str[0] == "-all":
        drgevent = await edit_or_reply(event, "__Enabling all safe cmds for sudo....__")
        ...
        for plugin in input_str:
            if plugin not in PLG_INFO:
                errors += f"`{cmd}` __There is no such plugin in your Dragons-Userbot__.\n"
            else:
        ...
        for cmd in input_str:
            if cmd not in CMD_INFO:
                errors += f"`{cmd}` __There is no such command in your Dragons-Userbot__.\n"
    ...
    result = f"__Successfully enabled __ `{len(loadcmds)}` __ for Dragons-Userbot sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
        output += "\n**Errors:**\n" + errors
    msg = await edit_or_reply(drgevent, output)
    await event.client.reload(msg)
...
async def _(event):  # sourcery no-metrics
    ...
    result = f"__Successfully disabled __ `{count}` __ for Dragons-Userbot sudo.__\n"
    output = (
        result + "**Bot is reloading to apply the changes. Please wait for a minute**\n"
    )
    if errors != "":
