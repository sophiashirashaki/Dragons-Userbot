...
from ..helpers.utils import _drgtools, reply_id
...
async def get_manga(event):
    ...
    drgevent = await edit_or_reply(event, "`Searching Manga..`")
...
async def get_manga(event):
    ...
    drgevent = await edit_or_reply(event, "`Searching Anime..`")
    jikan = jikanpy.jikan.Jikan()
...
async def character(event):
    ...
    drgevent = await edit_or_reply(event, "`Searching Character...`")
    try:
        search_result = jikan.search("character", search_query)
    except APIException:
        return await edit_delete(drgevent, "`Character not found.`")
    first_mal_id = search_result["results"][0]["mal_id"]
...
async def anime_download(event):  # sourcery no-metrics
    ...
    if input_str == "kaizoku":
        ...
        if search_result:
            ...
            result = f"<b>No result found for</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>IndiAnime</code>"
    await drgevent.edit(result, parse_mode="html")
...
async def whatanime(event):
    ...
    output = await _drgtools.media_to_pic(event, reply)
    if output[1] is None:
        return await edit_delete(
    ...
    drg = f"https://telegra.ph{response[0]}"
    await output[0].edit("`Searching for result..`")
    async with aiohttp.ClientSession() as session:
        async with session.post(
            f"https://api.trace.moe/search?anilistInfo&url={quote_plus(drg)}"
        ) as raw_resp0:
