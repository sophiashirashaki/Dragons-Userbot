...
from ..helpers.utils import _drgutils, _format
...
@drgub.drg_cmd(
    ...
    info={
        ...
        "usage": "{tr}ls <path>",
        "examples": "{tr}ls dragons",
    },
)
async def lst(event):  # sourcery no-metrics
    "To list all files and folders."
    drg = "".join(event.text.split(maxsplit=1)[1:])
    path = drg or os.getcwd()
    if not os.path.exists(path):
        await edit_or_reply(
            event,
            f"there is no such directory or file with the name `{drg}` check again",
        )
        return
    path = Path(drg) if drg else os.getcwd()
    if os.path.isdir(path):
        ...
        for contents in sorted(lists):
            drgpath = os.path.join(path, contents)
            if not os.path.isdir(drgpath):
                size = os.stat(drgpath).st_size
...
async def lst(event):
    ...
    drg = event.pattern_match.group(1)
    if drg:
        path = Path(drg)
    else:
...
async def _(event):
    ...
    try:
        await _drgutils.runcmd(f"mkdir {original}")
...
async def _(event):
    ...
    await asyncio.sleep(2)
    try:
        await _drgutils.runcmd(f"cp -r {original} {location}")
