...
from . import _format, drgub, deEmojify, edit_delete, edit_or_reply
...
async def figlet(event):
    ...
    if ";" in input_str:
        ...
        await edit_or_reply(event, "`Berikan saya beberapa teks`")
        return
    style = cmd
    text = text.strip()
    if style is not None:
        try:
            ...
            return await edit_delete(
                event, "**Tidak ada style yang ditemukan**, __Check__ `.info figlet`."
            )
