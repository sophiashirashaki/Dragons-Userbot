...
async def fetch_fedinfo(event):
    ...
    drgevent = await edit_or_reply(event, "`Fetching info about given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
            await conv.send_message("/fedinfo " + input_str)
            response = await conv.get_response()
            await drgevent.edit(response.text)
        except YouBlockedUserError:
            await edit_delete(
                drgevent,
            ...
            await edit_delete(
                drgevent, f"**Error while fecthing fedinfo:**\n__{e}__", 10
            )
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
...
async def fetch_fedinfo(event):
    ...
    drgevent = await edit_or_reply(event, "`Fetching admins list of given fed...`")
    async with event.client.conversation(rose) as conv:
        try:
...
async def myfeds_fedinfo(event):
    ...
    drgevent = await edit_or_reply(event, "`Fetching list of feds...`")
...
async def fstat_rose(event):
    ...
    drgevent = await edit_or_reply(event, "`Fetching fedstat from given deatils...`")
    user, fedid = await get_user_from_event(
        event, drgevent, secondgroup=True, noedits=True
    )
    if user is None:
        user = await event.client.get_me()
    if fedid is None:
    ...
    async with event.client.conversation(rose) as conv:
        try:
            ...
            if "Looks like" in response.message:
                ...
                await drgevent.delete()
                return
            await drgevent.edit(result + response.text)
        except YouBlockedUserError:
            await edit_delete(
                drgevent,
            ...
            await edit_delete(
                drgevent, f"**Error while fecthing fedstat:**\n__{e}__", 10
            )
        await event.client.send_read_acknowledge(conv.chat_id)
        conv.cancel()
