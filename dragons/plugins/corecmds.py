...
from . import CMD_HELP, CMD_LIST, SUDO_LIST, drgub, edit_delete, edit_or_reply, reply_id
...
@drgub.drg_cmd(
    ...
    info={
        ...
        "description": "Reply to any external plugin(supported by dragons) to install it in your bot.",
        "usage": "{tr}install",
    },
)
...
async def send(event):
    ...
    the_plugin_file = f"./dragons/plugins/{input_str}.py"
...
async def unload(event):
    ...
    shortname = event.pattern_match.group(1)
    path = Path(f"dragons/plugins/{shortname}.py")
