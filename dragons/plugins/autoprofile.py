...
async def autopicloop():
    ...
    if AUTOPICSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await dgrub.send_message(
                BOTLOG_CHATID,
    ...
    while AUTOPICSTART:
        ...
        file = await drgub.upload_file(autophoto_path)
        try:
            await drgub(functions.photos.UploadProfilePhotoRequest(file))
...
async def custompfploop():
    ...
    while CUSTOMPICSTART:
        ...
        file = await drgub.upload_file("donottouch.jpg")
        try:
            if i > 0:
                await drgub(
                    functions.photos.DeletePhotosRequest(
                        await drgub.get_profile_photos("me", limit=1)
                    )
                )
            i += 1
            await drgub(functions.photos.UploadProfilePhotoRequest(file))
...
async def digitalpicloop():
    ...
    while DIGITALPICSTART:
        ...
        img = Image.open(autophoto_path)
        drawn_text = ImageDraw.Draw(img)
        drg = str(base64.b64decode("dXNlcmJvdC9oZWxwZXJzL3N0eWxlcy9kaWdpdGFsLnR0Zg=="))[
            2:36
        ]
        fnt = ImageFont.truetype(drg, 200)
...
async def bloom_pfploop():
    ...
    if BLOOMSTART and Config.DEFAULT_PIC is None:
        if BOTLOG:
            return await drgub.send_message(
                BOTLOG_CHATID,
...
async def autoname_loop():
    ...
    while AUTONAMESTART:
        ...
        LOGS.info(name)
        try:
            await drgub(functions.account.UpdateProfileRequest(first_name=name))
...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        LOGS.info(bio)
        try:
            await drgub(functions.account.UpdateProfileRequest(about=bio))
        except FloodWaitError as ex:
            LOGS.warning(str(ex))
            await asyncio.sleep(ex.seconds)
        await asyncio.sleep(Config.CHANGE_TIME)
...
async def autopfp_start():
    ...
    while AUTOPFP_START:
        ...
        i += 1
        await drgub(functions.photos.UploadProfilePhotoRequest(file))
        await _drgutils.runcmd("rm -rf donottouch.jpg")
        await asyncio.sleep(Config.CHANGE_TIME)
...
drgub.loop.create_task(autopfp_start())
drgub.loop.create_task(autopicloop())
drgub.loop.create_task(digitalpicloop())
drgub.loop.create_task(bloom_pfploop())
drgub.loop.create_task(autoname_loop())
drgub.loop.create_task(autobio_loop())
drgub.loop.create_task(custompfploop())
