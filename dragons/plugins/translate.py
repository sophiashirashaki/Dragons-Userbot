...
@drgub.drg_cmd(
    ...
    info={
        "header": "untuk menerjemahkan bahasa.",
        "note": "untuk mengetahui kode bahasa klik [link ini](https://bit.ly/2SRQ6WU)",
        "usage": [
            "{tr}tl <kode bahasa> ; <text>",
            "{tr}tl <kode bahasa>",
        ],
        "examples": "{tr}tl id ; Dragons - Userbot is one of the popular bot",
    },
)
async def _(event):
...
@drgub.drg_cmd(
    ...
    info={
        "header": "untuk menerjemahkan bahasa yang diperlukan.",
    ...
    if BOTLOG:
        if input_str == "trt":
            await value.client.send_message(
                BOTLOG_CHATID, f"`Bahasa untuk translator diganti menjadi {LANG.title()}.`"
            )
        if input_str == "ai":
            await value.client.send_message(
                BOTLOG_CHATID, f"`Bahasa untuk chatbot diganti menjadi {LANG.title()}.`"
            )
