...
@drgub.drg_cmd(
    ...
    info={
        ...
        "examples": "{tr}msgto @Dragons_Userbot just a testmessage",
    },
)
async def drgbroadcast_add(event):
...
async def drgbroadcast_add(event):
    ...
    drginput_str = event.pattern_match.group(1)
    if not drginput_str:
        return await edit_delete(
            event,
...
async def drgbroadcast_list(event):
    ...
    await edit_or_reply(drgevent, finaloutput)
...
async def drgbroadcast_send(event):
...
async def drgbroadcast_remove(event):
...
async def drgbroadcast_delete(event):
    "To delete a category completely."
    drginput_str = event.pattern_match.group(1)
    check1 = sql.num_broadcastlist_chat(drginput_str)
    if check1 < 1:
        return await edit_delete(
            event,
            f"Are you sure that there is category {drginput_str}",
            parse_mode=_format.parse_pre,
        )
    try:
        sql.del_keyword_broadcastlist(drginput_str)
        await edit_or_reply(
            event,
            f"Successfully deleted the category {drginput_str}",
            parse_mode=_format.parse_pre,
        )
    except Exception as e:
        await edit_delete(
