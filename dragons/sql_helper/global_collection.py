...
class Drg_GlobalCollection(BASE):
    __tablename__ = "drg_globalcollection"
    ...
    def __repr__(self):
        return "<Dragons Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
...
def add_to_collectionlist(keywoard, contents):
    with DRG_GLOBALCOLLECTION:
        keyword_items = Drg_GlobalCollection(keywoard, tuple(contents))
